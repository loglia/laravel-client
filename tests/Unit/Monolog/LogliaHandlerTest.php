...
class LogliaHandlerTest extends TestCase
{
    ...
    public function it_sends_the_log_to_loglia_by_default()
    {
        ...
        $this->assertSame("curl -H 'Authorization: Bearer abc123' -H 'Content-Type: application/json' -A 'Loglia Laravel Client v2.0.0' -X POST -d '{\"hello\", \"world\"}' https://logs.loglia.app > /dev/null 2>&1 &", $handler->getLastCommand());
    }
    ...
    public function it_sends_the_log_to_different_endpoint_when_configured()
    {
        ...
        $this->assertSame("curl -H 'Authorization: Bearer abc123' -H 'Content-Type: application/json' -A 'Loglia Laravel Client v2.0.0' -X POST -d '{\"hello\", \"world\"}' https://example.org > /dev/null 2>&1 &", $handler->getLastCommand());
    }
    ...
    public function it_supports_unicode_characters_in_logging_payload()
    {
        ...
        $this->assertSame("curl -H 'Authorization: Bearer abc123' -H 'Content-Type: application/json' -A 'Loglia Laravel Client v2.0.0' -X POST -d '{\"unicode\", \"§Ĭɮڡউ█👍\"}' https://logs.loglia.app > /dev/null 2>&1 &", $handler->getLastCommand());
    }
}
